...
這不是你不會用，是 NotebookLM 的設計限制。以下是我實際踩過的 7 個問題，以及對應的解決方案。

## 常見問題與解決方案

### 1. PDF 亂碼（最常見）
* **原因：** PDF 內嵌字型沒有 ToUnicode 映射，AI 無法正確提取文字。
* **解決方案：** * **預處理：** 用 Acrobat 或線上工具將 PDF 轉為「標準字型」。
  * **改用純文字：** 直接複製文字貼到 `.txt` 檔上傳。
  * **避免掃描檔：** 圖片型 PDF 需先經過 OCR 工具（如 Adobe Scan）轉為可搜尋文字。

### 2. 高度排版文件辨識差
* **原因：** 解析器對複雜表格或多欄位排版處理能力有限。
* **解決方案：** 將表格轉成純文字列表，或直接上傳整理過的 `.csv` 與 Markdown 格式。

### 3. 檔案大小與數量限制
* **限制：** 單檔 200 MB、單一筆記本 50 個來源。
* **解決方案：** * **分割檔案：** 將大檔按章節拆分。
  * **精簡來源：** 優先放「高品質原始資料」而非二手整理。

### 4. AI 輸出不一致、有幻覺
* **原因：** LLM 的隨機性與 RAG 架構在多來源處理時的注意力分散。
* **解決方案：** * **指定來源：** 對話時勾選「只參考來源 A 和 B」。
  * **精煉提示詞：** 要求「根據第 X 頁內容回答」。
...
### 6. 繁體中文輸出異常
* **解決方案：** 在問題結尾加上「**請用繁體中文回答，保持台灣用語習慣**」。
...
經過測試，我建議這樣使用 NotebookLM 以發揮最大價值：

1. **Podcast 複習：** 上傳會議記錄，通勤時聽 AI 生成的對話。
2. **快速定位：** 在海量資料中搜尋「某概念出現在哪裡」。
3. **初步摘要：** 作為長文消化的第一輪草稿。

**不建議用於：** 精確報表分析、關鍵決策依據、複雜邏輯推理。
...
了解邊界後，才能真正發揮 AI 的整合威力：

* **[NotebookLM 跨來源整合實戰手冊](/blog/notebooklm-integration-guide)**：掌握了避坑指南，現在開始建立你的專案知識庫。
* **[PM 使用 AI 做數據分析的生存指南](/blog/pm-ai-data-survival-guide)**：當 AI 出現幻覺時，如何用你的專業判斷力站穩腳跟。
* **[常見問題 (FAQ)](/faqs)**：更多關於 AI 工具導入職場的疑慮與避坑指南。